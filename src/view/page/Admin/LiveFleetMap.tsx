...
export function LiveFleetMap() {
    ...
    // Combine fleet and demand markers
    const allMarkers = [
        ...activeTrips.map((t, index, array) => {
        ...
        ...demandSignals.map(s => ({
            lat: s.lat,
            lng: s.lng,
            label: `Demand Hotspot: ${s.reason === 'both' ? 'Total Outage' : s.reason === 'no_drivers' ? 'Driver Shortage' : 'Vehicle Scarcity'}`,
            details: `Area: ${s.address}\nTime: ${new Date(s.createdAt).toLocaleString()}\nLogged by: ${s.userId?.name || 'Anonymous'}`,
        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-4">
                    ...
                    <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-100 text-center">
                        <span className="block text-2xl font-bold text-red-600">{demandSignals.length}</span>
                        <span className="text-xs text-gray-500 uppercase font-bold">Hotspots</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden mb-8">
                <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">
                            {allMarkers.length > 0
                                ...
                                : "No active missions or demand signals detected."}
                        </p>
                    </div>
                    <div className="flex gap-4 text-[10px] font-bold uppercase tracking-wider">
                        ...
                        <div className="flex items-center gap-1.5">
                            <span className="w-2.5 h-2.5 bg-red-500 rounded-full inline-block shadow-sm"></span>
                            <span className="text-red-600">Demand Hotspot</span>
                        </div>
                    </div>
                </div>
